<div class="mt-12 w-100 flex flex-col items-center">
    ...
    <div class="w-3/6 flex justify-end">
        ...
        <div>
            <button
                wire:click='$emit("openModal", "raffle-partner-modal", @json(["action" => App\Http\Livewire\RafflePartnerModal::STORE_ACTION ]))'
                type="button"
    ...
    <table class="w-3/6 border-collapse text-center text-sm text-gray-500 mb-8 shadow-md">
        ...
        @if(sizeof($raffle_partners) > 0)
            @foreach($raffle_partners as $raffle_partner)
                <tr class="hover:bg-gray-200 font-bold even:bg-gray-100 odd:bg-gray-50">
                    <td class="px-6 py-4">{{ $raffle_partner->id }}</td>
                    <td class="px-6 py-4">{{ $raffle_partner->organization_name }}</td>
                    <td class="px-6 py-4">
                            <span
                                class="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-semibold {{  $raffle_partner->active ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600' }}"
                            >
                                <span class="h-1.5 w-1.5 rounded-full {{ $raffle_partner->active ? 'bg-green-600' : 'bg-red-600' }}"></span>
                                {{ $raffle_partner->active ? 'Active':'Inactive' }}
                            </span>
                    </td>
                    <td>
                        <button
                            wire:click='$emit("openModal", "raffle-partner-modal", @json(["action" => App\Http\Livewire\RafflePartnerModal::UPDATE_ACTION, "raffle_partner_id" => $raffle_partner->id ]))'
                            type="button"
                        ...
                        <a
                            href="{{ route('raffle.area', [ 'id' => $raffle_partner->id ]) }}"
                            type="button"
                        ...
                        >
                            <i class="text-base fa-solid fa-vault px-1"></i>
                        </a>
                    </td>
                </tr>
            @endforeach
