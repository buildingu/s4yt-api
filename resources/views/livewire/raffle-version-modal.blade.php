<x-modal form-action="processModal">
    <x-slot name="title">
        @if($action == \App\Http\Livewire\RaffleAreaModal::STORE_ACTION)
            Add raffle item to current version
        @else
            Edit {{ $name }} in current version
        @endif
    </x-slot>
        ...
        <div class="flex justify-center">
            <div class="w-full text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                ...
                <div class="flex flex-col">
                    <label class="leading-loose" for="stock">Stock <span class="align-super text-red-500">*</span></label>
                    <input
                        type="text"
                        id="stock"
                        name="stock"
                        class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                        wire:model.debounce.200ms="stock">
                    @error('stock')<small class="text-red-500 text-xs">{{ $message }}</small>@enderror
                </div>
                <!-- ACTIVE -->
